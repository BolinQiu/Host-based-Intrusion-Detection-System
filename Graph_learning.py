...
file_handler = logging.FileHandler(artifact_dir + 'training2.log')
...
def load_train_data():
    '''
        加载训练数据，数据文件为TemporalData.simple。
        
        :return: a list of TemporalData objects.
    '''

    training_set = []
    for day in training_day:
        training_set.append(torch.load(graphs_dir + f'/graph_{month}_{day}.TemporalData.simple').to(device=device))
    return training_set
...
if __name__ == "__main__":
    ...
    torch.save(model, f"{models_dir}/models2.pt")
